...
class Menu:
    ...
    def menu_prompt(self) -> str:
        """
        Display the available options and prompt the user for which option they will take.
        :return: The text to be executed in the calling function.
        """
        while True:
            print(self.prompt)  # Display the menu prompt
            for index, option in enumerate(self.options, start=1):  # Show the list of options
                print(f"{index} - {option.get_prompt()}")
            choice = input('--> ').strip()
            ...
            if 1 <= choice <= len(self.options):
                return self.options[choice - 1].get_action()
            else:
                print("Choice is out of range, try again.")


    def last_action(self):
        return self.options[-1].get_action()
    def last_action(self):
        """
